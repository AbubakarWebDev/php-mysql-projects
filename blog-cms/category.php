...
<section class="content container-fluid my-5">
            ...
            <?php 
                ...
                if ($result = mysqli_query($connection, $sql)) 
                {
                    if(mysqli_num_rows($result) > 0) 
                    {
                        ...
                        {
                            echo "<h3 class='text-uppercase' > {$row['category_name']} </h3>";
                        }
                    }
                }
                else {
    ...
    <div class='row'>
        ...
        <?php 
            ...
            $sql = "SELECT no_of_post FROM category WHERE category_id = {$category_id} ";
            ...
            if ($result = mysqli_query($connection, $sql)) 
            {
                ...
                if($row["no_of_post"] > 0) {
                    ...
                    if ($page_no > 1) {
                        echo "<span><a href='category.php?category_id={$category_id}&page_no={$prev_page}'><i class='fas fa-angle-left'></i></a></span>";
                    }
                    ...
                    for ($i=1; $i <= $total_pages; $i++) {
                        ...
                        echo "<span class='$active'><a href='category.php?category_id={$category_id}&page_no=$i'>$i</a></span>";
                    }

                    if ($page_no > 3) {
                        echo "
                            <div>....</div>
                            <span><a href='category.php?category_id={$category_id}&page_no={$total_pages}'>$total_pages</a></span>";                            
                    }
                    
                    if ($page_no != $total_pages) {
                        echo "<span><a href='category.php?category_id={$category_id}&page_no={$next_page}'><i class='fas fa-angle-right'></i></a></span>";
                    }
