        ...
        <?php 
            ...
            if ($result = mysqli_query($connection, $sql)) 
            {
                if(mysqli_num_rows($result) > 0) 
                {
                    ...
                    echo "<h3 class='text-center my-4'>Nothing to Show Here. No Blog Post is Present.</h3>";
                }
            }
            else 
            {
            ...
            if ($result = mysqli_query($connection, $sql)) 
            {
                if(mysqli_num_rows($result) > 0) {
                    ...
                    if ($page_no > 1) {
                        echo "<span><a href='?page_no={$prev_page}'><i class='fas fa-angle-left'></i></a></span>";
                    }
                    ...
                    for ($i=1; $i <= $total_pages; $i++) {
                        ...
                        echo "<span class='$active'><a href='?page_no=$i'>$i</a></span>";
                    }

                    if ($page_no > 3) {
                        echo "
                            <div>....</div>
                            <span><a href='?page_no={$total_pages}'>$total_pages</a></span>";                            
                    }
                    
                    if ($page_no != $total_pages) {
                        echo "<span><a href='?page_no={$next_page}'><i class='fas fa-angle-right'></i></a></span>";
                    }
