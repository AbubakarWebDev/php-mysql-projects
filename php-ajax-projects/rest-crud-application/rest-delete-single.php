<?php
    ...
    // Access-Control-Allow-Origin is a CORS (Cross-Origin Resource Sharing) response-type header. When Site A tries to fetch content from Site B, Site B can send an Access-Control-Allow-Origin header to tell the browser that the content of this page is accessible to certain origins. Below Header states that Our Rest Api is Accessed By Anyone.
    ...
    if ($result = mysqli_query($connection, $sql) or die(json_encode(array("error" => "Query Unsucessfull", "message" => mysqli_error($connection), "status" => false), JSON_PRETTY_PRINT))) {
        ...
        {
            die(json_encode(array("message" => "Your Entered ID Not Exist On Dataase. Please Try Different ID!", "status" => false), JSON_PRETTY_PRINT));
        }
    }

    // Sql Query For Delete Record On Database
    $sql = "DELETE FROM person WHERE ID = {$data['id']};";

    if ($result = mysqli_query($connection, $sql) or die(json_encode(array("error" => "Query Unsucessfull", "message" => mysqli_error($connection), "status" => false), JSON_PRETTY_PRINT))) 
    {
        echo json_encode(array("message" => "Your Record is Deleted Successfully!", "status" => true), JSON_PRETTY_PRINT);
    }
    else {
        label:
